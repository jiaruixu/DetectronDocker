...
_GOOGLEDRIVE_BASE_URL = 'https://drive.google.com/a/umich.edu'
...
def cache_url(url_or_file, cache_dir):
    ...
    assert url.startswith(_DETECTRON_S3_BASE_URL) or url.startswith(_GOOGLEDRIVE_BASE_URL), \
       ('Detectron only automatically caches URLs in the Detectron S3 and google drive '
        'bucket: {}').format(_DETECTRON_S3_BASE_URL)

    if url.startswith(_DETECTRON_S3_BASE_URL):
        cache_file_path = url.replace(_DETECTRON_S3_BASE_URL, cache_dir)
    else:
        cache_file_path = url.replace(_GOOGLEDRIVE_BASE_URL, cache_dir)
