...
def train_model():
    ...
    blob_names.remove('data')
    blob_names += ['roi_feat']
    frcnn_data_dict = {k:[] for k in blob_names}
    part_num = 1
    output_dir = get_output_dir(cfg.TRAIN.DATASETS, training=True)
    trainable_params = model.TrainableParams()
    logger.info('List of trainable params are: {}'.format(trainable_params))
    ### end added by srmani
        ...
        for gpu_id in range(cfg.NUM_GPUS):
            with c2_utils.NamedCudaScope(gpu_id):
                for blob_name in blob_names:
                    frcnn_data_dict[blob_name].append(workspace.FetchBlob(core.ScopedName(blob_name)))
                # print our the filename for debugging
                roidb = pickle.loads(frcnn_data_dict['roidb'][-1].astype(np.uint8).tobytes()) # get the last inserted roidb blob
                logger.info('Exported blobs for {}'.format(roidb[0]['image']))
        if cur_iter % 300 == 0:
            frcnn_data_filename = os.path.join(output_dir, 'frcnn_data_part{}.pkl'.format(part_num))
            save_object(frcnn_data_dict, frcnn_data_filename)
            part_num = part_num + 1
            frcnn_data_dict = {k:[] for k in blob_names}
        ### end added by srmani
