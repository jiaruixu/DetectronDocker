...
class RoIDataLoader(object):
    def __init__(
        ...
        if not cfg.TRAIN.EXPORT_FRCNN_DATA:
            self._shuffle_roidb_inds()
        else:
            logger.info('RoI data is NOT shuffled since EXPORT_FRCNN_DATA is set to true')
        self.create_threads()
        ...
        with self._lock:
            ...
            if self._cur >= len(self._perm):
                if cfg.TRAIN.EXPORT_FRCNN_DATA:
                    logger.info('Exported all images in dataset. Wrapping around since MAX_ITER({})>cur_iter({})'.format(cfg.SOLVER.MAX_ITER, self._cur))
                else:
                    self._shuffle_roidb_inds()
        return db_inds
