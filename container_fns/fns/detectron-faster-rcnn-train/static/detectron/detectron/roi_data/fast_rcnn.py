...
def get_fast_rcnn_blob_names(is_training=True):
    ...
    ### added by srmani
    if is_training and cfg.TRAIN.SOFT_SAMPLING_ON:
        blob_names += ['label_weights']
    ### end added by srmani
...
def _sample_rois(roidb, im_scale, batch_idx):
    ...
    if cfg.TRAIN.SOFT_SAMPLING_ON:
        if roidb['soft_sampling'] == 1:
            ...
            label_weights = np.ones(keep_inds.shape[0], dtype=max_overlaps.dtype)
