...
def add_fpn_retinanet_losses(model):
    ...
    for lvl in range(k_min, k_max + 1):
        ...
        if not cfg.RETINANET.SOFTMAX:
            ...
            ## added by Jiarui
            # zero_blob = model.net.ConstantFill(
            #             ['retnet_cls_labels_' + suffix,], 'zero_blob', value=0.0
            # )
            # cls_control_loss = model.net.L1Distance(
            #     [
            #         'retnet_cls_labels_' + suffix,
            #         zero_blob
            #     ],
            #     ['cl_{}'.format(suffix)],
            #     scale=cfg.RETINANET.LOSS_K
            # )
            # gradients.append(cls_control_loss)
            # losses.append('cl_{}'.format(suffix))
            # ## end added by Jiarui
        else:
